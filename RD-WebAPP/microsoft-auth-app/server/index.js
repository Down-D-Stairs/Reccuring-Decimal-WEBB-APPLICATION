...
app.post('/api/convert-pdf-preview', upload.single('pdf'), async (req, res) => {
  try {
    if (!req.file) {
    ...
    const outputDir = path.join(__dirname, 'temp', 'previews');
    ...
    const options = {
      format: 'jpeg',
      out_dir: outputDir,
      out_prefix: `preview_${Date.now()}`,
      page: 1, // First page only
      scale: 1024 // Good quality for preview
    };
    ...
    if (results && results.length > 0) {
      ...
      fs.unlinkSync(imagePath); // Remove generated image
    ...
    res.status(500).json({ 
      error: 'Failed to convert PDF to preview image',
